...
import yaml
from utils.functions import download_video_func

project_dir = os.getcwd()
with open(join(project_dir, 'config.yaml'), 'r') as f:
    cfg = yaml.load(f)

shoot_duration = cfg['auto_stream_desktop']['shoot_hours'] * 3600
...
while True:
    ...
    while True:
        ...
        if time() > start + shoot_duration:
            start = time()
            print("Time to download, prepare downloading videos!")
            download_video_func()
