...
def main(args):
    ...
    # subdir = '%s_center_loss_factor_%1.2f' % (subdir, args.center_loss_factor)

    # test = os.path.expanduser(args.logs_base_dir)
    log_dir = os.path.join(os.path.expanduser(cfg['fine_tune']['logs_base_dir']), subdir)
    if not os.path.isdir(log_dir):  # Create the log directory if it doesn't exist
        os.makedirs(log_dir)
    model_dir = os.path.join(os.path.expanduser(cfg['fine_tune']['models_base_dir']), subdir)
    ...
    train_set = facenet.get_dataset(cfg['fine_tune']['data_dir'])
    ...
    if args.lfw_dir:
        ...
        lfw_paths, actual_issame = lfw.get_paths_personal(os.path.expanduser(args.lfw_dir), pairs)
...
def parse_arguments(argv):
    ...
    #                     help='Load a pretrained model before training starts.', default='~/models/facenet/20180202-140429/model-20180202-140429.ckpt-117000')
    parser.add_argument('--pretrained_model', type=str,
                        help='Load a pretrained model before training starts.', default='~/models/model-20170511-185253.ckpt-80000')
    parser.add_argument('--data_dir', type=str,
                        help='Path to the data directory containing aligned face patches.',
                        default='~/data/facenet/train')
    ...
    parser.add_argument('--lfw_pairs', type=str,
                        help='The file containing the pairs to use for validation.', default='~/data/facenet/test/pairs.txt')
    parser.add_argument('--lfw_file_ext', type=str,
                        help='The file extension for the LFW dataset.', default='png', choices=['jpg', 'png'])
    parser.add_argument('--lfw_dir', type=str,
                        help='Path to the data directory containing aligned face patches.', default='~/data/facenet/test/')
